...
> **Performance Tip for Windows 11 Users:** Consider using [Dev Drive](https://learn.microsoft.com/en-us/windows/dev-drive/) to store your projects. Dev Drives are optimized for developer workloads and can significantly improve build times and disk access speeds by up to 30% compared to regular NTFS drives.
...
> **Other Templates:** Try `-t react`, `-t vue`, or `-t svelte` for your preferred framework. Run `wails3 init -l` to see all available templates.
...
> **First Run:** The first run may take longer than expected as it installs frontend dependencies, generates bindings, etc. Subsequent runs are much faster.
...
> **Understanding Services and Bindings:** Services are self-contained Go modules that expose functionality to your frontend. They're just regular Go structs with exported methods that you register in the `Services` field of your application config.
>
...
> **Where are the bindings?** Generated bindings are in `frontend/bindings/`. They're created automatically when you run `wails3 dev` or `wails3 build`.
>
...
> **Auto-Rebuild:** Go code changes trigger an automatic rebuild and restart. Frontend changes hot-reload without restart.
...
**Windows:** `bin/myapp.exe` - Double-click to run. No dependencies needed (WebView2 is part of Windows).

**macOS:** `bin/myapp.app` - Drag to Applications folder or double-click to run.

**Linux:** `bin/myapp` - Run with `./bin/myapp` or create a `.desktop` file for your launcher.

> **Cross-Platform Builds:** Want to build for other platforms? See [Cross-Platform Builds →](https://v3.wails.io/guides/build/cross-platform)
