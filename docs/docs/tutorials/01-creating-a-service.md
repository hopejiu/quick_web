...
> **Note:** The very first time you ever run this in a project, the bindings generator does a thorough analysis of your code and dependencies. This can sometimes take a little longer than expected, however subsequent runs will be much faster.
...
> **Pro Tip:** When you build your application using `wails3 build`, it will automatically generate bindings for you and keep them up to date.
...
> **Note:** It's not necessary to fully understand the generated bindings, but it's important to understand how they work.
...
> **TypeScript:** The bindings generator also supports generating Typescript bindings. You can do this by running `wails3 generate bindings -ts`.
...
> **Note:** Simplified import paths are only available when using frontend bundlers. If you prefer a vanilla frontend that does not employ a bundler, you will have to import either `index.js` or `qrservice.js` manually.
...
> **Note:** If you do not set the `Route` option explicitly, the HTTP handler won't be accessible from the frontend.
...
> **Note:** If a Go method returns a non-nil error, the promise on the JS side will reject and await statements will throw an exception.
