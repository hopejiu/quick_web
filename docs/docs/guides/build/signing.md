...
> **Tip:** Passwords are stored in your system's native credential store (macOS Keychain, Windows Credential Manager, or Linux Secret Service). This means your signing configuration is secure and works across all your Wails projects.
...
> **Tip:** For distribution outside the App Store, you need a **Developer ID Application** certificate.
...
> **Note:** The `run` task in the darwin Taskfile uses `entitlements.dev.plist` automatically. The `sign` tasks use `entitlements.plist` for production builds.
...
> **Note:** Notarization typically takes 1-2 minutes. The ticket is automatically stapled to your app.
...
> **Note:** The certificate password is stored in your system keychain, not in the Taskfile. Run `wails3 setup signing` to configure it, or set the `WAILS_WINDOWS_CERT_PASSWORD` environment variable in CI.
...
> **Caution:** Keep your private key secure! Store it encrypted and back it up safely.
...
> **Note:** The PGP key password is stored in your system keychain, not in the Taskfile. Run `wails3 setup signing` to configure it, or set the `WAILS_PGP_PASSWORD` environment variable in CI.
...
> **Note:** Using a Linux runner for cross-platform signing simplifies CI/CD by eliminating the need for separate Windows runners. macOS still requires a macOS runner for signing due to Apple's native tooling requirements.
