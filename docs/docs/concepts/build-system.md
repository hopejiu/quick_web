...
Project metadata (name, identifier, version, info-plist values, NSIS settings, `.desktop` fields, custom protocols, etc.) lives in `build/config.yml`. The Taskfile reads this file when generating icons, manifests, installers, and the like.
...
- **Building Applications** - Detailed guide to building and packaging
  [Learn More →](https://v3.wails.io/guides/building)
- **Cross-Platform Builds** - Build for all platforms from one machine
  [Learn More →](https://v3.wails.io/guides/cross-platform)
- **Creating Installers** - Create installers for end users
  [Learn More →](https://v3.wails.io/guides/installers)
